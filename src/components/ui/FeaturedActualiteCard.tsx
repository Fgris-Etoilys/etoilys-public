...
export default function FeaturedActualiteCard({ article }: FeaturedActualiteCardProps) {
  ...
  return (
    <Card as="article" hover={false} className="group">
      <Link
        to={article.href}
        aria-label={`Lire l’article ${article.title}`}
        className={`ui-focus rounded-editorial text-ink no-underline hover:text-ink ${
          hasImage ? 'grid lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]' : 'block'
        }`}
      >
        <div className="flex min-w-0 flex-col p-6 sm:p-8 lg:p-10">
          ...
          <h2 className="mb-4 text-h3 leading-tight text-ink">{article.title}</h2>
          <p className="mb-6 text-lg leading-comfortable text-muted">{article.excerpt}</p>
          <span
            aria-hidden="true"
            className="mt-auto inline-flex min-h-11 w-fit items-center gap-2 rounded-control bg-ink px-5 py-2.5 text-sm font-semibold text-white no-underline transition-colors duration-200 group-hover:bg-ink-hover group-hover:text-white motion-reduce:transition-none"
          >
            Lire l’article
