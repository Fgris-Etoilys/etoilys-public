...
type CityLandingPageVariant = 'v3' | 'v4';
...
const cityClassificationBenefits: ReasonCard[] = [
  {
    icon: Euro,
...
const etoilysV3PrimaryReason: ReasonCard = {
  icon: Sparkles,
...
const cofracReasonV3: ReasonCard = {
  icon: ShieldCheck,
...
const etoilysV3SecondaryReasons: ReasonCard[] = [
  {
    icon: FileCheck,
...
const cofracReasonV4: ReasonCard = {
  ...cofracReasonV3,
...
export default function CityLandingPage({ config }: CityLandingPageProps) {
  ...
  return (
    <>
      ...
      <ServiceAreaSection config={config} />
      <TaxSection config={config} />
      <LocalWarningSection config={config} />
      <ProcedureSection config={config} />
      <TariffsSection city={config.city} />
      <EtoilysReasonsSection city={config.city} variant="v3" />
      <FaqSection title={config.faq.title} items={faqItems} />
      <FinalCtaSection config={config} />
    </>
  );
...
function HeroSection({ config }: { config: CityLandingPageConfig }) {
  return (
    <section className="relative min-h-[720px] overflow-hidden py-section text-white">
      ...
      <div className="container-adaptive relative flex min-h-[560px] items-center">
        <div className="max-w-4xl">
          ...
          <p className="max-w-3xl text-xl leading-comfortable text-white/90">{config.hero.intro}</p>
      ...
      <div className="container-adaptive">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {cityClassificationBenefits.map((benefit) => (
    ...
    <section className="bg-primary-100 py-section">
      <div className="container-adaptive">
        <div className="mx-auto max-w-6xl">
          ...
          <h2 className="mb-5">Combien coûte le classement d’un meublé à {city} ?</h2>
...
function EtoilysReasonsSection({
  city,
  variant,
}: {
  city: string;
...
function ReasonCardItem({ reason }: { reason: ReasonCard }) {
  return (
    <Card hover={false} className="p-6">
      <ReasonIcon reason={reason} className="mb-4 h-8 w-8 text-primary-300" />
...
function ReasonIcon({ reason, className }: { reason: ReasonCard; className: string }) {
...
function FinalCtaSection({ config }: { config: CityLandingPageConfig }) {
  return (
