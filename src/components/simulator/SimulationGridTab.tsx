...
function RatioBar({
  ...
  return (
    ...
    >
      <div
        className={`h-2 rounded-full ${isValid ? 'bg-ink' : 'bg-copper'} transition-[width] duration-700 ease-out motion-reduce:transition-none`}
...
function ResultScoreCard({
  ...
  return (
    <div className="classement-result-score">
      <div className="space-y-4">
        <div>
          <p className="text-sm font-medium text-muted">{title}</p>
          <p className="mt-2 text-3xl font-semibold leading-none tracking-tight tabular-nums text-ink">
        ...
        <span
          className={`inline-flex w-fit items-center gap-2 text-sm font-medium ${
            hasReachedGoal ? 'text-success-500' : 'text-alert-500'
          }`}
        >
          {hasReachedGoal && <CheckCircle2 size={16} aria-hidden="true" />}
...
function GridTableOfContents({
  ...
  return (
    <>
      <div className="classement-grid-mobile-nav lg:hidden">
        <label
        ...
        <select
          ...
          className="ui-field min-w-0 max-w-full text-sm"
      ...
      <aside className="hidden lg:block lg:self-stretch">
        <div className="classement-grid-nav">
          <h3 className="mb-5 text-xs font-semibold uppercase tracking-[0.14em] text-muted">
            Sommaire
          </h3>
          <nav
          ...
          >
            {groups.map((group) => (
              <div key={group.chapterLabel} className="space-y-2">
                <p className="px-2 text-xs font-semibold leading-relaxed text-ink">
                  {group.chapterLabel}
                </p>
                <div className="space-y-1 border-l border-ink/15 pl-2">
                  {group.links.map((link) => {
                    ...
                    return (
                      <button
                        ...
                        className={`block min-h-11 w-full rounded-control px-3 py-2 text-left text-sm leading-snug transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-copper ${
                          isActive
          ...
          <Button
            type="button"
            variant="secondary"
            className="simulator-tool-link mt-4 w-full gap-2"
...
function ResponseButton({
  ...
  const selectedClassNames: Record<CriterionValidationStatus, string> = {
    VALIDE: 'border-success-500 bg-success-500 text-white hover:bg-success-500',
  ...
  return (
    <button
      ...
      className={`classement-response-button inline-flex min-h-11 items-center justify-center gap-1.5 rounded-control border px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-copper focus-visible:ring-offset-2 disabled:pointer-events-none disabled:cursor-default ${
...
function CriterionResponseButtons({
  ...
  return (
    <div
      ...
      className="classement-response-options"
    >
      <ResponseButton
        validation="VALIDE"
...
function GridCriterionCard({
  ...
  return (
    <article
      ...
      className="classement-criterion"
      tabIndex={-1}
      data-testid={`criterion-card-${criterion.num_critere}`}
    >
      <div className="classement-criterion-main">
        <div className="min-w-0">
          <div className="mb-2 flex flex-wrap items-center gap-x-3 gap-y-2">
            <p className="text-xs font-semibold tracking-wide text-muted">
              Critère {criterion.num_critere}
            </p>
            <span className="text-xs font-medium text-ink">{formatPoints(criterion.points)}</span>
            <span
              className={`inline-flex rounded-control border px-2 py-0.5 text-xs ${getCriterionStatusBadgeClassName(status)}`}
            >
              {status ? STATUS_LABELS[status] : 'Statut non renseigné'}
            </span>
          </div>
          <h6 className="text-base font-medium leading-relaxed text-ink">{criterion.libelle}</h6>
        </div>
        <button
          type="button"
          className="ui-focus inline-flex min-h-11 items-center justify-center gap-1.5 self-start rounded-control px-2 text-xs font-medium text-copper hover:bg-paper"
      ...
      <div className="classement-criterion-answer">
        {isSurfaceReadOnly && (
          <>
            <AutomaticCriterionStatus response={response} />
            <p className="mt-2 text-xs leading-relaxed text-muted">{SURFACE_CRITERION_MESSAGE}</p>
          </>
        )}

        {isBusinessNotApplicable && !isSurfaceReadOnly && (
          <p className="rounded-control bg-paper px-3 py-2 text-xs leading-relaxed text-muted">
...
function GridChapterSection({
  ...
  if (chapters.length === 0) {
    return (
      <Card
        hover={false}
        className="flex min-h-44 flex-col items-center justify-center gap-3 p-6 text-center"
      >
        <Search size={24} className="text-copper" aria-hidden="true" />
  ...
  return (
    <div className="space-y-12">
      {chapters.map((chapter, chapterIndex) => (
        ...
        >
          <div className="flex items-start gap-3 border-b border-ink/20 pb-4">
            <span
              className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ink text-sm font-medium text-paper"
              aria-hidden="true"
            >
              {chapterIndex + 1}
            </span>
            <h3
              ...
              className="pt-0.5 text-xl font-semibold leading-snug tracking-tight text-ink"
            >
              {chapter.libelle}
            </h3>
          </div>
          ...
          <div className="space-y-8">
            {chapter.sous_chapitres.map((subChapter, subChapterIndex) => (
              <section
                ...
                id={getSectionAnchorId(chapter.libelle, subChapter.libelle)}
                className="classement-grid-section space-y-4"
                ...
                {subChapter.rubriques.map((rubrique) => (
                  <div key={`${subChapter.libelle}-${rubrique.libelle}`} className="space-y-3">
                    {rubrique.libelle.trim() && (
                      <h5 className="text-xs font-semibold uppercase tracking-[0.1em] text-muted">
                        {rubrique.libelle}
                      </h5>
                    )}
                    <div className="classement-criterion-list">
...
export function SimulationVerificationIssues({
  ...
  return (
    <div className="classement-result space-y-5">
      <div
        className="rounded-editorial border border-warning-200 bg-warning-100/60 p-5 text-sm font-medium leading-relaxed text-ink md:p-6"
        role="alert"
      >
      ...
      <Card hover={false} className="p-5 md:p-7">
        <h3 className="mb-4 text-lg font-semibold text-ink">Problèmes à corriger</h3>
...
export function SimulationResultPanel({
  ...
  const resultToneClassNames = success
    ? {
        shell: 'bg-surface-sage',
        title: 'text-ink',
        badge: 'bg-white/80 text-success-500',
      }
    : {
        shell: 'bg-surface-warm',
        title: 'text-ink',
        badge: 'bg-white/80 text-alert-500',
      };
  const verdictRole = success ? 'status' : 'alert';

  return (
    <>
      <div className="classement-result">
        <div
          className={`classement-result-verdict ${resultToneClassNames.shell}`}
          role={verdictRole}
        >
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 flex-col">
              <p className="simulator-eyebrow">Résultat de la simulation</p>
              <h3
                className={`max-w-2xl font-playfair text-3xl font-medium leading-tight tracking-tight md:text-4xl ${resultToneClassNames.title}`}
              >
                {success
            ...
            <div className="flex shrink-0 flex-col items-start gap-4 lg:items-end">
              <span
                className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${resultToneClassNames.badge}`}
              >
        ...
        <div className="classement-result-scores">
          <ResultScoreCard
        ...
        {shouldShowDiagnostic && (
          <div className="classement-result-diagnostic">
            <h4 className="text-lg font-semibold text-ink">Critères obligatoires non validés</h4>
        ...
        <div className="flex flex-col gap-3 border-t border-ink/10 px-5 py-5 sm:flex-row md:px-7">
          <Button
            type="button"
            variant="secondary"
      ...
      <div className="simulator-next editorial-focus-inverse">
        <div>
          <h2>Prêt pour une visite officielle ?</h2>
          <p>
        ...
        <div className="simulator-next-actions">
          ...
          <Button href="/faq" variant="secondary" className="simulator-next-secondary">
            Consulter la FAQ
          </Button>
        </div>
      </div>
...
export default function SimulationGridTab({
  ...
  return (
    <div id="grid-top" className="classement-grid space-y-7">
      <div className="classement-grid-summary">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 max-w-2xl">
            <span className="simulator-eyebrow block">ÉTAPE 2 — GRILLE DE CONTRÔLE</span>
            <h2 className="mb-3 text-2xl font-semibold tracking-tight text-ink">
              Complétez la grille de contrôle
            </h2>
            ...
            <div className="mt-4 text-ink">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-ink">Besoin d’aide sur un critère ?</p>
          ...
          <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:flex-wrap lg:flex-col lg:flex-nowrap">
            {!isGridComplete && (
              <Button
                type="button"
                variant="primary"
                className="min-h-11 w-full shrink-0 gap-3 sm:w-auto"
            ...
            <Button
              ...
              className={`min-h-11 w-full shrink-0 sm:w-auto ${isGridComplete ? '' : 'bg-white'}`}
        ...
        <div className="classement-grid-progress">
          <dl className="flex flex-wrap items-end justify-between gap-4 text-sm">
            <div>
              <dt className="text-xs text-muted">Critères renseignés</dt>
            ...
            <div className="text-right">
              <dt className="text-xs text-muted">Critères restants</dt>
              <dd className="mt-1 text-xl font-semibold tabular-nums text-ink">
          ...
          <progress
            className="classement-grid-progress-bar"
            aria-label="Progression de la grille"
            value={progressSummary.answeredCount}
            max={progressSummary.totalCount || 1}
          />
        </div>
      </div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[220px_minmax(0,1fr)] xl:gap-9">
        <GridTableOfContents
        ...
        <div className="min-w-0 space-y-6">
          <div className="classement-grid-search">
            <Input
              ref={searchInputRef}
            ...
            {searchQuery && (
              <button
                type="button"
                className="ui-focus inline-flex min-h-11 shrink-0 items-center gap-2 rounded-control px-3 text-sm text-ink hover:bg-surface"
                onClick={() => {
                  setSearchQuery('');
          ...
          {isCriterionFilterActive && (
            <div
              className="flex flex-col gap-3 border-l-2 border-copper bg-surface-warm p-4 text-sm text-ink sm:flex-row sm:items-center sm:justify-between"
              role="status"
            >
              <p>
          ...
          <div className="flex flex-col justify-end gap-3 border-t border-ink/15 pt-6 sm:flex-row">
            <Button
              type="button"
              variant="primary"
              className="w-full sm:w-auto"
